...
URL=https://$HOST/DAV/home/demo/outbox/
...
TOKEN=$(awk -v ATTR=access_token -f token.awk -f JSON.awk oauth_token.json)
TOKEN_TYPE=$(awk -v ATTR=token_type -f token.awk -f JSON.awk oauth_token.json)
...
echo "Performing $action to [$URL] using Token: $TOKEN_TYPE $TOKEN"
curl -s -i -H"Authorization: Bearer $TOKEN" \
    -X POST -H"Content-Type: application/activity+json" \
    "$URL" --data-binary "$(makeAP "$action" "$arg")"
exit 0
